...
export const AvailabilityPostRequestSchema = zod.object({
  ...
  roleTypeNames: zod.array(zod.string()).min(1, { message: 'Please provide exactly one role type' }).max(1, { message: 'Please provide exactly one role type' }),
...
export const CycleSubspanWithJoinsListDtoSchema = zod.object({
  ...
  name: zod.string(),
  cycleSubspanJoins: zod.record(zod.number().int(), zod.lazy(() => CycleSubspanJoinNestedDtoSchema)),
  zeroIndexedCycleDay: zod.number().int(),
  dayOrdinal: zod.number().int(),
  joinsIfNotFirst: zod.record(zod.number().int(), zod.array(zod.lazy(() => CycleSubspanJoinNestedDtoSchema))),
});

export const HierarchyEnrollmentRequestSchema = zod.object({
  scheduleId: zod.number().int(),
  organizationIdToUserRoleIdList: zod.record(zod.number().int(), zod.array(zod.number().int())),
});
...
export const WorkProjectSeriesWithSchemaLabelsDtoSchema = zod.object({
  ...
  workProjectSeriesSchema: zod.lazy(() => WorkProjectSeriesSchemaWithLabelsDtoSchema),
  workTaskType: zod.lazy(() => WorkTaskTypeDtoSchema),
});
...
export const WorkTaskTypeListMatrixSchema = zod.object({
  knowledgeLevelSeriesDtoList: zod.array(zod.lazy(() => KnowledgeLevelSeriesDtoSchema)).min(1, { message: 'Please supply at least oneknowledgeLevelSeries' }),
  knowledgeDomainDtoList: zod.array(zod.lazy(() => KnowledgeDomainDtoSchema)).min(1, { message: 'Please supply at least oneknowledgeDomain' }),
  workTaskTypeNames: zod.array(zod.string()).min(1, { message: 'Please supply at least oneworkTaskTypeName' }),
});

export const WorkTaskTypeResourceRequirementPostRequestSchema = zod.object({
  resourceRequirementItemRequests: zod.array(zod.lazy(() => ResourceRequirementItemRequestSchema)),
  workTaskTypeMatrix: zod.lazy(() => WorkTaskTypeListMatrixSchema),
});
...
export const SchoolYearPlanInitDtoSchema = zod.object({
  yearLessonAllocationPlans: zod.record(zod.number().int(), zod.record(zod.string(), zod.lazy(() => LessonAllocationInitSchema))),
...
export const CycleDtoSchema = zod.object({
  ...
  cycleDayZero: zod.enum(['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY', 'SUNDAY']),
  cycleLengthInWeeks: zod.number().int().min(1, { message: 'Cycle length must be at least 1 week' }),
});
...
export const CarouselOrderDtoSchema = zod.object({
  id: zod.string().uuid(),
  carouselOrderItems: zod.record(zod.number().int(), zod.lazy(() => CarouselOrderItemDtoSchema)),
  userRoleId: zod.number().int(),
  carouselGroupId: zod.number().int(),
});
...
export const OrganizationDtoSchema = zod.object({
  ...
  workSchemaNodeAssignment: zod.lazy(() => WorkSchemaNodeAssignmentDtoSchema.nullable()),
});
...
export const QueueTreeNodeDtoSchema = zod.object({
  ...
  workProjectSeriesNodeLinks: zod.array(zod.lazy(() => WorkProjectSeriesNodeLinkDtoSchema)),
});

export const WorkProjectSeriesSchemaDtoSchema = zod.object({
  id: zod.number().int(),
  name: zod.string(),
  deliveryAllocations: zod.record(zod.number().int(), zod.lazy(() => DeliveryAllocationDtoSchema)),
  workProjectBandwidth: zod.number().int(),
  userToProviderRatio: zod.number(),
  workTaskType: zod.lazy(() => WorkTaskTypeDtoSchema),
});
...
export const WorkSchemaNodeAssignmentDtoSchema = zod.object({
  ...
  workSchemaNodeId: zod.number().int().nullable(),
});
...
export const WorkSchemaNodeDtoSchema = zod.object({
  ...
  carouselOptionId: zod.number().int().nullable(),
  id: zod.number().int(),
  knowledgeDomainId: zod.number().int().nullable(),
  knowledgeLevelId: zod.number().int().nullable(),
  name: zod.string(),
  carouselGroupId: zod.number().int().nullable(),
  carouselId: zod.number().int().nullable(),
  workProjectSeriesSchemaId: zod.number().int().nullable(),
  resolutionMode: zod.enum(['OPEN', 'LEAF', 'CAROUSEL', 'SERIAL', 'CAROUSEL_OPTION', 'CAROUSEL_GROUP']),
});
...
export const WorkTaskTypeDtoSchema = zod.object({
  name: zod.string(),
  id: zod.number().int(),
  knowledgeDomain: zod.lazy(() => KnowledgeDomainDtoSchema.nullable()),
  knowledgeLevel: zod.lazy(() => KnowledgeLevelDtoSchema.nullable()),
  validationType: zod.lazy(() => ValidationTypeDtoSchema.nullable()),
  knowledgeLevelSeriesId: zod.number().int().nullable(),
});
